...
# Drop all columns except for body (user-generated comments) and trait relevant for fine-tuning (Openness in this case)
df.drop(['author', 'ext', 'neu', 'agr', 'con'], axis=1, inplace=True)
...
class RoBERTaForPersonalityTraits(torch.nn.Module):
    def __init__(self):
        ...
        self.linear = torch.nn.Linear(768, 1)  # Output size for 1 personality trait
...
batch_size = 16
train_dataset = PersonalityDataset(df_train['body'].tolist(), df_train['ope'].values)
val_dataset = PersonalityDataset(df_val['body'].tolist(), df_val['ope'].values)
test_dataset = PersonalityDataset(df_test['body'].tolist(), df_test['ope'].values)
...
# Output log file
log_file = r'/roberta_base_o_result.txt'  
...
with open(log_file, 'w') as f:  
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_progress_bar):
            ...
            if step % 5000 == 0:
                torch.save(model.state_dict(), f'/roberta_base_o/model_step_{step}.pth')
        ...
        with torch.no_grad():
            for batch in val_progress_bar:
                ...
                #for i in range(5):
                all_predictions[i].extend(outputs.cpu().numpy())
                all_targets[i].extend(targets.cpu().numpy())
        ...
        if avg_val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), r'/roberta_base_o_best_model.pth')  # Save best model
        else:
            patience_counter += 1
    ...
    # Save final model
    torch.save(model.state_dict(), r'roberta_base_o_final_model.pth')
