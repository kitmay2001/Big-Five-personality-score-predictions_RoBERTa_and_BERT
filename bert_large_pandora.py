...
df_train, df_val = train_test_split(df_ train, test_size=0.2, random_state=42)
...
# Define the model
class BERTForPersonalityTraits(torch.nn.Module):
    def __init__(self):
        super(BERTForPersonalityTraits, self).__init__()
...
model = BERTForPersonalityTraits()
...
# Output log file
log_file = r'/bert_large_result.txt'  
...
with open(log_file, 'w') as f:  
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_progress_bar):
            ...
            if step % 5000 == 0:
                torch.save(model.state_dict(), f'bert_large/model_step_{step}.pth')
        ...
        if avg_val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), r'/bert_large_best_model.pth')  # Save best model
        else:
            patience_counter += 1
    ...
    # Save final model
    torch.save(model.state_dict(), r'/bert_large_final_model.pth')
